...
##Summary
TailorUP, a Ruby on Rails Web Application designed to create an easy interactive
 alternative for customers to request a tailor appointment for fitting, monitor
 progression of alterations, and receive delivered garments without having to
...
Monday-Saturday 10am-6pm (Shop Hours) then this would allow them to create their
appointment listing out the type of garments needing alterations. The User
would receive updates via email and application when the tailor is on the way to
the for fittings, when alteration status are updated, and also once alterations
are delivered.
...
The company interface also has the capability to export a CSV listing every
